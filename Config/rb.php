...
namespace RedBeanPHP {
	 ...
	 * Logging interface for RedBeanPHP ORM
	 * 
...
namespace RedBeanPHP {
	 ...
	 * Describes the API for database classes
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
	...
	class RPDO implements Driver {
		...
		public function connect() {
			...
			try {
				...
				$this->pdo = new \PDO ( $this->dsn, $user, $pass, array (
...
namespace RedBeanPHP {
	 ...
	 * The Bean class used for passing information
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP community
	 * @license BSD/GPLv2
	 *         
	...
	class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable {
		...
		/**
		 * Unsets a property.
		 * This method will load the property first using
		 * __get.
		 *
		 * @param string $property
		 *        	property
		...
		public function beau($property) {
			...
			if (strpos ( $property, 'own' ) === 0 || strpos ( $property, 'xown' ) === 0 || strpos ( $property, 'shared' ) === 0) {
...
namespace RedBeanPHP {
	 ...
	 * ription Part of the observer pattern in RedBean
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP {
	 ...
	 * Describes the API for a RedBean Database Adapter.
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP\QueryWriter {
	 ...
	 * Represents a MySQL Database to RedBean
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
	...
	class MySQL extends AQueryWriter implements QueryWriter {
		...
		protected function constrain($table, $table1, $table2, $property1, $property2) {
			try {
				...
				CONSTRAINT_NAME <>'PRIMARY' AND REFERENCED_TABLE_NAME IS NOT NULL", array (
						$db,
						$table 
				) );
...
namespace RedBeanPHP\QueryWriter {
	 ...
	 * Represents a SQLite Database to RedBean
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP\QueryWriter {
	 ...
	 * QueryWriter for the PostgreSQL database system.
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
	...
	class PostgreSQL extends AQueryWriter implements QueryWriter {
		...
		public function scanType($value, $flagSpecial = FALSE) {
			...
			if ($value === NULL || ($value instanceof NULL) || (is_numeric ( $value ) && floor ( $value ) == $value && $value < 2147483648 && $value > - 2147483648)) {
				return self::C_DATATYPE_INTEGER;
...
namespace RedBeanPHP {
	 ...
	 * Represents the base class for RedBean\Exceptions
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP {
	...
	class OODB extends Observable {
		...
		private function handleException(\Exception $exception) {
			if ($this->isFrozen || ! $this->writer->sqlStateIn ( $exception->getSQLState (), array (
		...
		private function addForeignKeysForParentBeans($bean, $embeddedBeans) {
			...
			foreach ( $embeddedBeans as $linkField => $embeddedBean ) {
				...
				if (! isset ( $cachedIndex [$key] )) {
					$this->writer->addIndex ( $bean->getMeta ( 'type' ), 'index_foreignkey_' . $beanType . '_' . $embeddedType, $linkField );
...
namespace RedBeanPHP {
	 ...
	 * A RedBeanPHP-wide service locator
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP {
	...
	class Finder {
		...
		/**
		 *
		 * @see Finder::find The variation also exports the beans (i.e. it returns arrays).
		 *     
		 * @param string $type
		 *        	type the type of bean you are looking for
		 * @param string $sql
		...
		/**
		 *
		 * @see Finder::find This variation returns the first bean only.
		 *     
		 * @param string $type
		 *        	type the type of bean you are looking for
		 * @param string $sql
		...
		/**
		 *
		 * @see Finder::find This variation returns the last bean only.
		 *     
		 * @param string $type
		...
		/**
		 *
		 * @see Finder::find Convience method. Tries to find beans of a certain type,
...
namespace RedBeanPHP\BeanHelper {
	 ...
	 * Finds the toolbox for the bean.
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP {
	 ...
	 * Connects beans to models, in essence
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
...
namespace RedBeanPHP {
	 ...
	 * Convenience class for RedBeanPHP.
	 * 
	 * @author Gabor de Mooij and the RedBeanPHP Community
	 * @license BSD/GPLv2
	 *         
